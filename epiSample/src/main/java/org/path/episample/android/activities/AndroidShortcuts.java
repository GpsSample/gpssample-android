...
public class AndroidShortcuts extends Activity {
  ...
  private void buildMenuList() {
    Bitmap appIcon = BitmapFactory.decodeResource(getResources(), R.drawable.ic_episample);
    Bitmap formIcon = BitmapFactory.decodeResource(getResources(), R.drawable.ic_episample_form);

    AlertDialog.Builder builder = new AlertDialog.Builder(this);
    builder.setTitle("Select EpiSample Shortcut");
    ...
    for (File app : directories) {
      ...
      Uri uri = Uri.withAppendedPath(FormsProviderAPI.CONTENT_URI, appName);
      choices.add(new Choice(R.drawable.ic_episample, appIcon, uri, "EpiSample", appName));
      ...
      try {
        ...
        if (c != null && c.getCount() > 0) {
          c.moveToPosition(-1);
          while (c.moveToNext()) {
            String formName = "EpiSample > "
                + ODKDatabaseUtils.get().getIndexAsString(c, c.getColumnIndex(FormsColumns.DISPLAY_NAME));
            if(formName.equalsIgnoreCase("EpiSample > " + CensusDatabaseHelper.CENSUS_DATABASES_TABLE) == false ) {
	            ...
	            choices.add(new Choice(R.drawable.ic_episample_form, formIcon, uri, formName, appName));
            }
          }
        }
      } finally {
