...
class DoubleRuleFactory {
    enum class Rules(val displayName: String, val comparator: (Double, Double) -> Boolean) {
    ...
    companion object: RuleMaker<Rules, Double> {
        override fun makeRule(ruleType: Rules, forField: CustomField, value: Double): Rule {
            return DoubleComparisonRule(ruleType, forField, DoubleValue(value))
        }
    }
}