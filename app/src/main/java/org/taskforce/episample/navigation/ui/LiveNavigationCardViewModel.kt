...
class LiveNavigationCardViewModel(application: Application,
    ...
    override val itemData = MutableLiveData<CollectItem>()
    ...
    override val buttonLayoutVisibility = MutableLiveData<Boolean>().apply { value = true }
    override val primaryButtonText: LiveData<String?> = Transformations.map(languageService.getString(R.string.launch_survey)) {
        return@map it.toUpperCase()
    }
    override val secondaryButtonText: LiveData<String?> = Transformations.map(languageService.getString(R.string.skip)) {
        return@map it.toUpperCase()
    }

    override fun primaryButtonAction(view: View) {
        launchSurvey()
    }
