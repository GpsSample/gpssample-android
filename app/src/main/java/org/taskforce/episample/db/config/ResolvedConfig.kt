...
abstract class ResolvedConfigDao {
    ...
    @Query("SELECT * from config_table c INNER JOIN user_settings_table us ON us.user_settings_config_id = c.id INNER JOIN admin_settings_table a ON a.admin_settings_config_id = c.id INNER JOIN enumeration_subject_table e ON e.enumeration_subject_config_id = c.id WHERE c.id LIKE :configId")
