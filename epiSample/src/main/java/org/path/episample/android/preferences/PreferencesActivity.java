...
import org.path.episample.android.activities.AccountList;
...
public class PreferencesActivity extends PreferenceActivity implements OnPreferenceChangeListener {
  ...
  private EditTextPreference mInterviewerNamePreference;
  private EditTextPreference mPhoneNumberPreference;
  private EditTextPreference mDeviceNamePreference;
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    setTitle("EpiSample > " + getString(R.string.general_preferences));
    ...
    PreferenceCategory interviewerDeviceInfoCategory = (PreferenceCategory) findPreference(getString(R.string.interviewer_device_info));
    
    // Initialize Interviewer Name Preference
    mInterviewerNamePreference = (EditTextPreference) findPreference(KEY_INTERVIEWER_NAME);
    if (PropertiesSingleton.containsKey(mAppName, KEY_INTERVIEWER_NAME)) {
      String interviewerName = PropertiesSingleton.getProperty(mAppName, KEY_INTERVIEWER_NAME);
      mInterviewerNamePreference.setSummary(interviewerName);
      mInterviewerNamePreference.setText(interviewerName);
    }
    
    mInterviewerNamePreference.setOnPreferenceChangeListener(this);
    
    boolean interviewerNameAvailable = (PropertiesSingleton.getProperty(mAppName,
            AdminPreferencesActivity.KEY_INTERVIEWER_NAME)).equalsIgnoreCase("true") ? true : false;
    if (!(interviewerNameAvailable || adminMode)) {
    	interviewerDeviceInfoCategory.removePreference(mInterviewerNamePreference);
    }
    
    // Initialize Phone Number Preference
    mPhoneNumberPreference = (EditTextPreference) findPreference(KEY_PHONE_NUMBER);
    if (PropertiesSingleton.containsKey(mAppName, KEY_PHONE_NUMBER)) {
        String phoneNumber = PropertiesSingleton.getProperty(mAppName, KEY_PHONE_NUMBER);
        mPhoneNumberPreference.setSummary(phoneNumber);
        mPhoneNumberPreference.setText(phoneNumber);
    }
    
    mPhoneNumberPreference.setOnPreferenceChangeListener(this);
    
    boolean phoneNumberAvailable = (PropertiesSingleton.getProperty(mAppName,
            AdminPreferencesActivity.KEY_PHONE_NUMBER)).equalsIgnoreCase("true") ? true : false;
    if (!(phoneNumberAvailable || adminMode)) {
    	interviewerDeviceInfoCategory.removePreference(mPhoneNumberPreference);
    }
    
    // Initialize Device Name Preference
    mDeviceNamePreference = (EditTextPreference) findPreference(KEY_DEVICE_NAME);
    if (PropertiesSingleton.containsKey(mAppName, KEY_DEVICE_NAME)) {
        String deviceName = PropertiesSingleton.getProperty(mAppName, KEY_DEVICE_NAME);
        mDeviceNamePreference.setSummary(deviceName);
        mDeviceNamePreference.setText(deviceName);
    }
    
    mDeviceNamePreference.setOnPreferenceChangeListener(this);
    
    boolean deviceNameAvailable = (PropertiesSingleton.getProperty(mAppName,
            AdminPreferencesActivity.KEY_DEVICE_NAME)).equalsIgnoreCase("true") ? true : false;
    if (!(deviceNameAvailable || adminMode)) {
    	interviewerDeviceInfoCategory.removePreference(mDeviceNamePreference);
    }
    
    if (!(interviewerNameAvailable || phoneNumberAvailable || deviceNameAvailable || adminMode)) {
        getPreferenceScreen().removePreference(interviewerDeviceInfoCategory);
    }
